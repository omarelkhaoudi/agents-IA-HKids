...
interface SalesGenerationResponse {
  document: EntityResult;
}

interface SalesQuotationGenerationResponse {
  quotation: EntityResult;
}

interface SalesSearchResponse {
  items: Array<EntityResult & { type: string; title: string }>;
}

export async function getSalesAgentBootstrap() {
  return apiRequest<Record<string, any>>('/api/sales-agent/bootstrap');
}

export async function searchSalesAgent(query: string) {
  return apiRequest<SalesSearchResponse>(`/api/sales-agent/search?q=${encodeURIComponent(query)}`);
}

export async function createSalesCompany(payload: Record<string, unknown>) {
  return apiRequest<EntityResult>('/api/sales-agent/companies', {
    method: 'POST',
...
export async function createSalesProspect(payload: Record<string, unknown>) {
  return apiRequest<EntityResult>('/api/sales-agent/prospects', {
    method: 'POST',
    body: JSON.stringify(payload),
  });
}
...
export async function createSalesProduct(payload: Record<string, unknown>) {
  return apiRequest<EntityResult>('/api/sales-agent/products', {
    method: 'POST',
    body: JSON.stringify(payload),
  });
}
...
export async function createSalesDeal(payload: Record<string, unknown>) {
  return apiRequest<EntityResult>('/api/sales-agent/deals', {
    method: 'POST',
    body: JSON.stringify(payload),
  });
}
...
export async function moveSalesDeal(id: string, stage: string) {
  return apiRequest<EntityResult>(`/api/sales-agent/deals/${id}/move`, {
    method: 'POST',
...
export async function generateSalesDocument(payload: Record<string, unknown>) {
  return apiRequest<SalesGenerationResponse>('/api/sales-agent/generate', {
    method: 'POST',
    body: JSON.stringify(payload),
  });
}
...
export async function generateSalesQuotation(payload: Record<string, unknown>) {
  return apiRequest<SalesQuotationGenerationResponse>('/api/sales-agent/generate-quotation', {
    method: 'POST',
    body: JSON.stringify(payload),
  });
}
...
export async function submitSalesQuotationReview(id: string) {
  return apiRequest<EntityResult>(`/api/sales-agent/quotations/${id}/submit-review`, { method: 'POST' });
}

export async function approveSalesQuotation(id: string) {
  return apiRequest<EntityResult>(`/api/sales-agent/quotations/${id}/approve`, { method: 'POST' });
}

export async function rejectSalesQuotation(id: string) {
  return apiRequest<EntityResult>(`/api/sales-agent/quotations/${id}/reject`, { method: 'POST' });
}

export async function submitSalesDocumentReview(id: string) {
  return apiRequest<EntityResult>(`/api/sales-agent/documents/${id}/submit-review`, { method: 'POST' });
}

export async function approveSalesDocument(id: string) {
  return apiRequest<EntityResult>(`/api/sales-agent/documents/${id}/approve`, { method: 'POST' });
}

export async function rejectSalesDocument(id: string) {
  return apiRequest<EntityResult>(`/api/sales-agent/documents/${id}/reject`, { method: 'POST' });
}
